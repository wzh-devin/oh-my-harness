...
/** 有限获取技能仓库，禁用 hooks、子模块及重定向，并保留取消边界。 */
...
export async function extractZip(
  ...
  if (bytes.length > 64 * 1024 * 1024)
    throw new SkillError('SKILL_ZIP_TOO_LARGE', 'ZIP 不能超过 64 MiB')
  ...
  try {
    await new Promise<void>((done, fail) => {
      abort = () => fail(new SkillError('SKILL_IMPORT_CANCELLED', '导入已取消'))
      ...
      zip.on('entry', (entry: yauzl.Entry) => {
        entryWork = (async () => {
          ...
          if (
            ...
            throw new SkillError('SKILL_ZIP_TOO_LARGE', 'ZIP 条目超出解压限制')
            ...
            transform(chunk: Buffer, _encoding, callback) {
              ...
              callback(
                total > MAX_BYTES || size > MAX_FILE_BYTES
                  ? new SkillError('SKILL_ZIP_TOO_LARGE', 'ZIP 超出解压限制')
                  : null,
                chunk,
              )
            },
