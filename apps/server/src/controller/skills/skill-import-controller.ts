import { SkillError } from '@oh-my-harness/agent-runtime'
import type {
  AgentRuntime,
  SkillImportService,
...
import type {
  SkillImportDto,
  SkillDetailDto,
} from '../../dto/skills/skill-import-dto.ts'
...
const field = (data: unknown, key: string) => {
  ...
  if (
    ...
    throw new SkillError('SKILL_REQUEST_INVALID', '请求字段无效')
  return value.trim()
}
  ...
  const protect =
    (operation: (c: Context) => Promise<Response>) => async (c: Context) => {
      try {
        ...
        return c.json(
          error instanceof SkillError || error instanceof AgentRuntimeError
